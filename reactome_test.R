reactome_test<-function(GO_terms,groups_length,col.label,col.map,num,filename,workingdirectory,up_reg,down_reg,other_reg)
{
    ...
    pathway<-read.delim(paste(workingdirectory,"/Functional_Searches/reactome_in_pathways_WHOLEGENOME_numbers.txt",sep=""),header=F)
    ...
    {
        pdf(paste(workingdirectory,"/",filename,"/","Significantly_enriched_reactome.pdf",sep=""))
        heatmap.2(heatmap_matrix,col=redgreen(75),scale="row",key=T,keysize=1.5,density.info="none",trace="none",cexCol=0.9,cexRow=0.5,labRow=row_names_matrix,labCol=1:groups_length,ColSideColors=col.map,main=c(paste("pattern ",num-1," Significantly Enriched Reactome",sep="")," ",col.label))
        dev.off()
    }
    ...
    if(length(up_reg) == 1)
    {
        ...
        {
            pdf(paste(workingdirectory,"/",filename,"/","Significantly_enriched_reactome(2).pdf",sep=""))
            heatmap.2(down_reg_matrix,col=redgreen(75),scale="row",key=T,keysize=1.5,density.info="none",trace="none",cexCol=0.9,cexRow=0.6,labRow=names_down,labCol=1:groups_length,ColSideColors=col.map,main=c(paste("pattern ",num-1," Significantly Enriched Reactome(2)",sep="")," ",col.label))
            dev.off()
        }
    }
    ...
    if(length(down_reg)==1)
    {
        ...
        {
            pdf(paste(workingdirectory,"/",filename,"/","Significantly_enriched_reactome(1).pdf",sep=""))
            heatmap.2(up_reg_matrix,col=redgreen(75),scale="row",key=T,keysize=1.5,density.info="none",trace="none",cexCol=0.9,cexRow=0.4,labRow=names_up,labCol=1:groups_length,ColSideColors=col.map,main=c(paste("pattern ",num-1," Significantly Enriched Reactome(1)",sep="")," ",col.label))
            dev.off()
        }
    }
    ...
    else
    {
        ...
        {
            pdf(paste(workingdirectory,"/",filename,"/","Significantly_enriched_reactome(2).pdf",sep=""))
            heatmap.2(down_reg_matrix,col=redgreen(75),scale="row",key=T,keysize=1.5,density.info="none",trace="none",cexCol=0.9,cexRow=0.4,labRow=names_down,labCol=1:groups_length,ColSideColors=col.map,main=c(paste("pattern ",num-1," Significantly Enriched Reactome (2)",sep="")," ",col.label))
            dev.off()
        }
        ...
        {
            pdf(paste(workingdirectory,"/",filename,"/","Significantly_enriched_reactome(3).pdf",sep=""))
            heatmap.2(other_reg_matrix,col=redgreen(75),scale="row",key=T,keysize=1.5,density.info="none",trace="none",cexCol=0.9,cexRow=0.4,labRow=names_other,labCol=1:groups_length,ColSideColors=col.map,main=c(paste("pattern ",num-1," Significantly Enriched Reactome (3)",sep="")," ",col.label))
        }
    }
    
    out<-c(names_up,names_down,names_other)
    identity_gene<-read.delim(paste(workingdirectory,"/Functional_Searches/reactome_in_pathways_WHOLEGENOME.txt",sep=""))
    ...
    write.table(out1,file = paste(workingdirectory,"/",filename,"/Reactome_Test_Results.txt",sep=""), row.names = FALSE)
}