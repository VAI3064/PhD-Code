pathways <- function(genes_vec,workingdirectory,all_orthologs)
{
    pathway<-read.delim(paste(workingdirectory,"/GO.txt",sep=""),header=T)
    pathway<-pathway[-1,]
    ...
    {
        pp<-which(pathway2[,2]==unique_pathways[i])
        numbers<-c()
        genes_in_pathway1<-c()
